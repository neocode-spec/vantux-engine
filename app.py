...
st.markdown("""
    ...
    .spin-animation { animation: galaxy-spin 2s linear infinite; display: inline-block; }
    
    /* GRADIENT LOGO & GLOWING SPARKLE */
    ...
    .blue-sparkle { font-size: 40px; background: linear-gradient(90deg, #60a5fa 0%, #3b82f6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    </style>
""", unsafe_allow_html=True)

# --- 3. BACKEND SETUP ---
SYSTEM_PROMPT = "You are Oremi, the Sovereign What-If Simulation Engine. You are allocated under the Prime Corporation division."
...
if "logged_in" not in st.session_state:
    st.session_state.update({"logged_in": False, "active_messages": [], "is_thinking": False})

# --- 4. HEADER LOGIC ---
# If thinking, apply the spin-animation class
sparkle_class = "blue-sparkle"
if st.session_state.get("is_thinking"):
    sparkle_class = "blue-sparkle spin-animation"
...
if not st.session_state["logged_in"]:
    ...
    if st.button("Transmit to Core") and prompt:
        ...
        st.rerun() # Trigger the spin

    # Only process if in thinking state
    if st.session_state.get("is_thinking"):
        try:
